...
DiscoursePluginRegistry.serialized_current_user_fields << "disable_signatures"
...
after_initialize do
  ...
  User.register_custom_field_type('disable_signatures', :boolean)
  User.register_custom_field_type('signature_url', :text)
  User.register_custom_field_type('signature_raw', :text)

  register_editable_user_custom_field [:disable_signatures, :signature_url, :signature_raw]
